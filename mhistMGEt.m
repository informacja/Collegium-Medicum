...
for(i=1:La)
    ...
    if(isinf(sx)||isnan(sx)) sx=0; end, J(i)=sx; if(sx<Jmin) Jmin=sx; aOpt=xa; end
        xa=xa+da;
end, % nbm=i;ysr
a=aOpt;
for(i=1:ldx)
    ...
    ft=tpdf(x,10); % rozklad t-Studenta
    ...
    NttSt(i)=ft*Dy*ldanych;
end
plot(xt,Nteorg,'r', xt, Ntexp,'g',xt,NtMBn,'b',xt,NttSt,'y'); %wyrysowanie wynikow
